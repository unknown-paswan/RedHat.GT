...
def detect_eyes(cascade_left, cascade_right, gray_frame):
    # Adjust the scaleFactor and minNeighbors parameters for better eye detection
    left_eye = cascade_left.detectMultiScale(gray_frame, scaleFactor=1.3, minNeighbors=5, minSize=(20, 20))
    right_eye = cascade_right.detectMultiScale(gray_frame, scaleFactor=1.3, minNeighbors=5, minSize=(20, 20))
...
def main():
    ...
    while True:
        ...
        # Detect eyes in the frame and check if they are closed
        left_eye, right_eye = detect_eyes(eye_cascade_left, eye_cascade_right, gray_frame)
        eyes_closed = len(left_eye) == 0 or len(right_eye) == 0
